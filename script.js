...
const path = d3.geoPath().projection(projection);
  ...
  .style("opacity", 0);
    ...
    Promise.all([
      ...
      d3.csv("public/gender_pay_gap_by_state.csv")
...
function drawBarChart() {
  d3.csv("public/Median weekly earnings of full-time wage and salary workers by occupation, 2023.csv").then(data => {
    data.forEach(d => {
      d.Women = +d.Women;
      d.Men = +d.Men;
    });
...
function drawEarningsChangeDivergingChart() {
  d3.csv("public/gender_pay_gap_education_2023.csv").then(data => {
    data.forEach(d => {
      d.Men = +d.Men;
      d.Women = +d.Women;
    });
...
function drawDotPlotEarningsByEducation() {
  d3.csv("public/earningsbysexandeducationalattainment.csv").then(data => {
